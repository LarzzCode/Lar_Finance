...
import { anomalyQueryStart, buildSpendingAnomalies, formatInsightDate } from '../lib/anomalyDetectionV36';
...
export default function SpendingInsightsV36() {
  const { user } = useAuth();
    ...
    <main className="min-h-screen pb-32 md:pb-16 pt-8 md:pt-32 text-slate-900 dark:text-slate-100">
      <div className="max-w-6xl mx-auto px-4 md:px-8">
        <header className="mb-8">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 max-w-3xl leading-relaxed">Lar Finance membaca pengeluaran sejak tanggal 1 bulan ini sampai hari ini, lalu membandingkannya dengan rentang tanggal yang sama pada 6 bulan sebelumnya. Median dipakai sebagai baseline agar satu bulan ekstrem tidak mendistorsi pola normal.</p>
        </header>
        ...
        <section className="grid md:grid-cols-3 gap-4 mb-5">
          <div className="liquid-nav rounded-[2rem] p-5 md:p-6">
            <p className="text-[10px] uppercase tracking-wide text-slate-400">Bulan berjalan</p>
          ...
          <div className="liquid-nav rounded-[2rem] p-5 md:p-6">
            ...
            <p className="text-[11px] text-slate-400 mt-2">Median 6 bulan pembanding</p>
          </div>
          <div className="liquid-nav rounded-[2rem] p-5 md:p-6">
        ...
        <section className="liquid-nav rounded-[2.3rem] p-6 md:p-8 mb-5">
          ...
          {loading ? <div className="h-32 rounded-2xl bg-white/30 dark:bg-white/[.03] animate-pulse" /> : !analysis.hasEnoughOverallHistory ? (
            <div className="rounded-2xl bg-indigo-500/[.06] p-5 text-sm text-slate-600 dark:text-slate-300"><SearchCheck size={20} className="text-indigo-500 mb-3" />Histori pembanding belum cukup konsisten. Lar Finance membutuhkan setidaknya 3 bulan pembanding berisi transaksi agar perbandingan total lebih bermakna.</div>
            ...
            <div className="space-y-3">
              {analysis.anomalies.map((item) => {
                ...
                return <article key={item.key} className="rounded-[1.7rem] border border-white/60 dark:border-white/10 bg-white/38 dark:bg-white/[.03] p-5">
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3"><div><div className="flex items-center gap-2 flex-wrap"><h3 className="text-lg font-semibold">{item.name}</h3><span className={`text-[10px] rounded-full px-2.5 py-1 font-medium ${meta.className}`}>{meta.label}</span></div><p className="text-xs text-slate-400 mt-1">{item.transactionCount} transaksi bulan ini · aktif pada {item.activeHistoryMonths}/6 bulan pembanding</p></div><div className="sm:text-right"><p className="text-xl font-semibold text-rose-500">{rupiah(item.current)}</p><p className="text-[11px] text-slate-400 mt-1">biasanya {rupiah(item.baseline)}</p></div></div>
        ...
        <section className="liquid-nav rounded-[2.3rem] p-6 md:p-8">
          <div className="flex items-center gap-3 mb-6"><BarChart3 size={20} className="text-indigo-500" /><div><p className="text-[10px] uppercase tracking-[0.15em] text-slate-400">Category baseline</p><h2 className="text-xl font-semibold mt-1">Kategori bulan ini</h2></div></div>
