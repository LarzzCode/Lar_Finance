...
export default function HomeV23() {
  ...
  const insight = useMemo(() => {
    if (loading) return 'Menganalisis data keuangan bulan ini…';
    if (!transactionCount) return 'Catat transaksi pertama agar Lar Finance mulai membaca pola cashflow kamu.';
    if (summary.income_month > 0 && summary.expense_month > summary.income_month) return 'Pengeluaran bulan ini sudah melewati pemasukan. Cek laporan mingguan dan budget sebelum menambah pengeluaran baru.';
    if (expenseRatio >= 80) return 'Lebih dari 80% pemasukan bulan ini sudah terpakai. Cek minggu dan kategori dengan pengeluaran terbesar.';
    if (!budgets.length) return 'Cashflow sudah tercatat. Langkah berikutnya: buat budget untuk kategori pengeluaran utama.';
    return 'Pengeluaran bulan ini masih terpantau. Jaga ritme pencatatan dan cek ringkasan mingguan secara berkala.';
    ...
    <main className="min-h-screen bg-[#F7F8FA] text-slate-900 pb-28 md:pb-16 pt-7 md:pt-32">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="rounded-[2rem] md:rounded-[2.5rem] bg-[#0B1220] text-white p-6 md:p-8 lg:p-10 shadow-2xl shadow-slate-900/10 overflow-hidden relative mb-6">
          <div className="absolute -right-28 -top-28 h-72 w-72 rounded-full bg-rose-500/15 blur-3xl" />
          <div className="absolute -left-24 -bottom-28 h-64 w-64 rounded-full bg-indigo-500/10 blur-3xl" />
          <div className="relative z-10 grid lg:grid-cols-[1.08fr_0.92fr] gap-8 lg:items-end">
            <div>
              <div className="flex items-center justify-between max-w-2xl mb-3">
                <div>
                  <p className="text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">Pengeluaran bulan ini</p>
                  <p className="text-xs text-slate-500 mt-1">Fokus dashboard hanya pada aktivitas bulan berjalan.</p>
                </div>
                <button onClick={() => setHideAmount((value) => !value)} className="p-2.5 rounded-full bg-white/5 hover:bg-white/10" aria-label={hideAmount ? 'Tampilkan nominal' : 'Sembunyikan nominal'}>
                  {hideAmount ? <Eye size={17} /> : <EyeOff size={17} />}
                </button>
              </div>
              <p className="font-black tracking-tight text-4xl md:text-6xl text-white">
                {hideAmount ? '••••••••' : loading ? '…' : rupiah(summary.expense_month)}
              </p>
              <div className="mt-5 inline-flex items-center gap-2 rounded-full bg-white/[0.06] border border-white/10 px-3 py-2 text-xs font-bold text-slate-300">
                <ReceiptText size={14} className="text-rose-300" />
                Rasio terhadap pemasukan {summary.income_month > 0 ? `${Math.round(expenseRatio)}%` : '—'}
              </div>
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-2xl border border-white/10 bg-white/[0.05] p-4 md:p-5">
                <div className="flex items-center gap-2 text-emerald-300 mb-2"><ArrowUpRight size={16} /><span className="text-[10px] font-black uppercase tracking-wider">Pemasukan bulan ini</span></div>
                <p className="font-black text-base md:text-lg truncate">{hideAmount ? '••••' : loading ? '…' : rupiah(summary.income_month)}</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/[0.05] p-4 md:p-5">
                <div className={`flex items-center gap-2 mb-2 ${monthNet >= 0 ? 'text-indigo-300' : 'text-rose-300'}`}>{monthNet >= 0 ? <ArrowUpRight size={16} /> : <ArrowDownRight size={16} />}<span className="text-[10px] font-black uppercase tracking-wider">Net bulan ini</span></div>
                <p className={`font-black text-base md:text-lg truncate ${monthNet < 0 ? 'text-rose-300' : ''}`}>{hideAmount ? '••••' : loading ? '…' : rupiah(monthNet)}</p>
              </div>
            </div>
          </div>
        </motion.section>
        ...
        {onboardingDone < onboarding.length && (
          <section className="mb-6 rounded-[2rem] border border-indigo-100 bg-indigo-50/70 p-5 md:p-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-5">
              <div><p className="text-[11px] font-black uppercase tracking-[0.16em] text-indigo-500">Quick setup</p><h2 className="text-xl font-black mt-1">Siapkan fondasi keuanganmu.</h2></div>
        ...
        <section className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-7">
          ...
          <Link to="/rekap" className="rounded-3xl border border-slate-200 bg-white p-5 hover:shadow-lg transition-shadow"><ReceiptText size={20} className="text-rose-500 mb-4" /><p className="font-black text-sm">Laporan</p><p className="text-xs text-slate-400 mt-1">Ringkasan mingguan</p></Link>
