...
export default function SubscriptionsV2() {
  ...
  const items = useMemo(() => {
    return subscriptions.map((sub) => {
      const isPaid = transactions.some(
        (tx) => (tx.description || '').trim().toLowerCase() === (sub.name || '').trim().toLowerCase()
      );
  ...
  const handleDelete = async (item) => {
    if (!window.confirm(`Hapus langganan ${item.name}?`)) return;
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8">
        <header className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-5 mb-7">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-2">Pantau beban bulanan dan catat pembayaran tanpa lupa jatuh tempo.</p>
          </div>
          <button onClick={() => setModalOpen(true)} className="inline-flex items-center justify-center gap-2 px-5 py-3.5 rounded-2xl bg-slate-950 text-white text-sm font-bold shadow-lg shadow-slate-900/10">
        ...
        <section className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-6">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5"><CalendarClock size={18} className="text-slate-500 mb-3" /><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-400">Belum lunas</p><p className="font-black text-lg md:text-xl mt-1">{stats.unpaidCount} tagihan</p></div>
        </section>
          ...
          <div className="p-5 md:p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="inline-flex bg-slate-100 p-1 rounded-2xl self-start">
              {[
                ...
                <button key={value} onClick={() => setFilter(value)} className={`px-4 py-2 rounded-xl text-xs font-bold transition ${filter === value ? 'bg-white shadow-sm text-slate-950' : 'text-slate-400'}`}>{label}</button>
              ))}
            </div>
          </div>
          ...
          <div className="p-4 md:p-6">
            {loading ? (
              ...
              <div className="py-14 text-center">
                <CalendarClock size={28} className="mx-auto text-slate-300 mb-4" />
                <h3 className="font-black">Belum ada tagihan di filter ini</h3>
                <p className="text-sm text-slate-500 mt-2">Tambahkan langganan rutin agar jatuh temponya mudah dipantau.</p>
              </div>
            ) : (
              <div className="grid md:grid-cols-2 gap-4">
                {visible.map((item) => (
                  <motion.div layout key={item.id} className="border border-slate-200 rounded-[1.75rem] p-5 bg-white hover:shadow-lg hover:shadow-slate-900/5 transition-shadow">
                    <div className="flex items-start justify-between gap-4 mb-5">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                          <span className={`inline-flex px-2.5 py-1 rounded-full border text-[10px] font-bold ${item.status.className}`}>{item.status.label}</span>
                          <span className="text-[10px] font-bold text-slate-400">Tgl {item.due_date}</span>
                        </div>
                        <h3 className="font-black text-lg truncate">{item.name}</h3>
                        <p className="text-xs text-slate-400 mt-1">{item.categories?.name || 'Tanpa kategori'} · {item.payment_method || 'cash'}</p>
                      </div>
                    ...
                    {item.isPaid ? (
                      <div className="w-full h-11 rounded-2xl bg-emerald-50 text-emerald-600 flex items-center justify-center gap-2 text-sm font-bold"><CheckCircle2 size={17} /> Sudah lunas</div>
                    ) : (
                      <button onClick={() => handlePay(item)} className="w-full h-11 rounded-2xl bg-slate-950 text-white text-sm font-bold hover:bg-black transition-colors">Catat sebagai dibayar</button>
                    )}
                  </motion.div>
                ))}
              </div>
          ...
          <div className="fixed inset-0 z-[100] bg-slate-950/45 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
            <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 30, opacity: 0 }} className="bg-white w-full sm:max-w-lg rounded-t-[2rem] sm:rounded-[2rem] p-6 md:p-7 shadow-2xl max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-6">
                <div><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-slate-400">Recurring bill</p><h3 className="text-xl font-black">Tagihan baru</h3></div>
              ...
              <form onSubmit={handleAdd} className="space-y-4">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div><label className="text-[11px] font-bold text-slate-500">Nominal</label><input required type="number" min="1" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} placeholder="0" className="mt-1 w-full p-4 rounded-2xl bg-slate-50 border border-slate-200 outline-none font-bold" /></div>
                  <div><label className="text-[11px] font-bold text-slate-500">Tanggal jatuh tempo</label><input required type="number" min="1" max="31" value={form.due_date} onChange={(e) => setForm({ ...form, due_date: e.target.value })} className="mt-1 w-full p-4 rounded-2xl bg-slate-50 border border-slate-200 outline-none font-bold" /></div>
                </div>
                ...
                <div><label className="text-[11px] font-bold text-slate-500">Metode bayar</label><input value={form.payment_method} onChange={(e) => setForm({ ...form, payment_method: e.target.value })} placeholder="cash / transfer" className="mt-1 w-full p-4 rounded-2xl bg-slate-50 border border-slate-200 outline-none" /></div>
                <button type="submit" className="w-full h-12 rounded-2xl bg-slate-950 text-white font-bold">Simpan tagihan</button>
              </form>
            </motion.div>
          </div>
        )}
