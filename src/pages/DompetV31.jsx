...
import { ArrowRightLeft, MoreHorizontal, Pencil, Plus, ShieldCheck, Trash2, WalletCards, X } from 'lucide-react';
...
export default function DompetV31() {
  const { user } = useAuth();
  ...
  const load = async () => {
    ...
    setWallets(calculateWalletBalances(walletRes.data || [], txRes.data || [], transferRes.data || []));
    setLoading(false);
  };
  ...
  const summary = useMemo(() => wallets.reduce((acc, wallet) => ({
    ...
    transferIn: acc.transferIn + Number(wallet.month_transfer_in || 0),
    transferOut: acc.transferOut + Number(wallet.month_transfer_out || 0),
  }), { balance: 0, monthIncome: 0, monthExpense: 0, transferIn: 0, transferOut: 0 }), [wallets]);
  ...
  const remove = async (wallet) => {
    ...
    const error = txRes.error || transferOutRes.error || transferInRes.error;
    if (error) return toast.error(error.message);
    const refs = (txRes.count || 0) + (transferOutRes.count || 0) + (transferInRes.count || 0);
    ...
    <main className="min-h-screen pb-32 md:pb-16 pt-8 md:pt-32 text-slate-900 dark:text-slate-100">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <header className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-5 mb-8">
          <div><p className="text-[10px] font-medium uppercase tracking-[0.18em] text-slate-400 mb-2">Wallet overview</p><h1 className="text-[2rem] md:text-[2.6rem] font-semibold tracking-[-0.04em]">Dompet saya</h1><p className="text-sm text-slate-500 dark:text-slate-400 mt-2">Saldo bulan ini sudah memperhitungkan transfer antar dompet tanpa mengubah cashflow.</p></div>
          <div className="flex flex-wrap gap-2"><Link to="/transfer" className="liquid-nav inline-flex items-center justify-center gap-2 rounded-2xl px-5 py-3.5 text-xs font-semibold"><ArrowRightLeft size={16} /> Transfer</Link><button onClick={openCreate} className="liquid-primary inline-flex items-center justify-center gap-2 text-white rounded-2xl px-5 py-3.5 text-xs font-semibold"><Plus size={16} /> Tambah dompet</button></div>
        </header>

        <section className="grid lg:grid-cols-[1.2fr_.8fr] gap-4 mb-7">
          <div className="relative overflow-hidden rounded-[2.2rem] border border-white/10 bg-slate-950/90 text-white p-6 md:p-8 shadow-[0_20px_70px_rgba(15,23,42,.18)] backdrop-blur-2xl"><div className="absolute -right-20 -top-20 w-56 h-56 rounded-full bg-indigo-400/20 blur-3xl" /><div className="relative"><p className="text-[10px] font-medium uppercase tracking-[0.18em] text-slate-400 mb-3">Total saldo bulan ini</p><p className={`text-4xl md:text-5xl font-semibold tracking-[-0.04em] ${summary.balance < 0 ? 'text-rose-300' : 'text-white'}`}>{loading ? '…' : rupiah(summary.balance)}</p><p className="text-xs text-slate-400 mt-3">Transfer antar dompet saling mengimbangi, jadi total saldo keseluruhan tidak berubah.</p></div></div>
          <div className="grid grid-cols-2 gap-4"><div className="liquid-nav rounded-[1.8rem] p-5"><p className="text-[9px] font-medium uppercase tracking-[0.15em] text-emerald-600">Masuk bulan ini</p><p className="text-lg font-semibold mt-2 text-emerald-600">+{rupiah(summary.monthIncome)}</p></div><div className="liquid-nav rounded-[1.8rem] p-5"><p className="text-[9px] font-medium uppercase tracking-[0.15em] text-rose-500">Keluar bulan ini</p><p className="text-lg font-semibold mt-2 text-rose-500">-{rupiah(summary.monthExpense)}</p></div><div className="col-span-2 liquid-nav rounded-[1.8rem] p-5 flex gap-3"><ShieldCheck size={18} className="text-indigo-500 shrink-0" /><div><p className="font-semibold text-sm">Safe wallet guard</p><p className="text-xs text-slate-400 mt-1">Dompet dengan histori transaksi atau transfer tetap terlindungi dari penghapusan.</p></div></div></div>
        </section>
        ...
        {loading ? <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">{[1,2,3].map((item) => <div key={item} className="h-48 liquid-nav rounded-[2rem] animate-pulse" />)}</div> : !wallets.length ? <div className="liquid-nav rounded-[2rem] py-16 text-center"><WalletCards size={30} className="mx-auto text-slate-300 mb-4" /><p className="font-semibold">Belum ada dompet.</p><button onClick={openCreate} className="mt-4 text-sm font-medium text-indigo-600">+ Tambah dompet pertama</button></div> : (
          <section className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {wallets.map((wallet, index) => <motion.article key={wallet.id} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }} className="liquid-nav rounded-[2rem] p-6 relative">
              ...
              <div className="mt-6"><p className="text-sm text-slate-500 dark:text-slate-400">{wallet.name}</p><p className={`text-2xl md:text-3xl font-semibold tracking-[-0.03em] mt-1 ${wallet.month_balance < 0 ? 'text-rose-500' : ''}`}>{rupiah(wallet.month_balance)}</p><p className="text-[10px] text-slate-400 mt-1">Saldo bulan ini</p></div>
              <div className="grid grid-cols-2 gap-3 mt-6 pt-5 border-t border-white/50 dark:border-white/10"><div><p className="text-[9px] uppercase tracking-wider text-slate-400">Transfer masuk</p><p className="text-xs font-semibold text-indigo-600 dark:text-indigo-300 mt-1">+{rupiah(wallet.month_transfer_in)}</p></div><div><p className="text-[9px] uppercase tracking-wider text-slate-400">Transfer keluar</p><p className="text-xs font-semibold text-slate-500 mt-1">-{rupiah(wallet.month_transfer_out)}</p></div></div>
            </motion.article>)}
          </section>
        )}
      </div>

      <AnimatePresence>{modalOpen && <div className="fixed inset-0 z-[90] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-950/35 backdrop-blur-md"><motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 30 }} className="liquid-nav w-full max-w-md rounded-t-[2rem] sm:rounded-[2rem] p-6 md:p-7"><div className="flex items-center justify-between mb-6"><div><p className="text-lg font-semibold">{editData ? 'Edit dompet' : 'Dompet baru'}</p><p className="text-xs text-slate-400 mt-1">Saldo awal menjadi baseline bulan berjalan.</p></div><button onClick={close} className="w-9 h-9 rounded-full bg-white/45 dark:bg-white/[.05] flex items-center justify-center"><X size={17} /></button></div><form onSubmit={save} className="space-y-5"><label className="block"><span className="text-[10px] font-medium uppercase tracking-wider text-slate-400">Nama dompet</span><input autoFocus required maxLength={40} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="BCA, Tunai, SeaBank..." className="w-full mt-2 rounded-2xl px-4 py-4 font-medium outline-none" /></label><label className="block"><span className="text-[10px] font-medium uppercase tracking-wider text-slate-400">Saldo awal bulan</span><input required type="number" value={form.saldo_awal} onChange={(e) => setForm({ ...form, saldo_awal: e.target.value })} placeholder="0" className="w-full mt-2 rounded-2xl px-4 py-4 font-medium outline-none" /></label><div className="grid grid-cols-2 gap-3"><button type="button" onClick={close} className="h-12 rounded-2xl bg-white/45 dark:bg-white/[.05] font-medium text-sm">Batal</button><button disabled={saving} className="liquid-primary h-12 rounded-2xl text-white font-semibold text-sm disabled:opacity-60">{saving ? 'Menyimpan…' : 'Simpan'}</button></div></form></motion.div></div>}</AnimatePresence>
    </main>
  );
}
