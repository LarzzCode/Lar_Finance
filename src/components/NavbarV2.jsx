...
const isPathActive = (pathname, path) => {
  ...
  if (path === '/planning') {
    return ['/planning', '/budget', '/subscription', '/savings', '/categories'].includes(pathname);
  }
...
export default function NavbarV2() {
  ...
  return (
    <>
      <div className="hidden md:flex fixed top-5 left-1/2 -translate-x-1/2 z-50 w-[min(94vw,980px)] justify-center pointer-events-none">
        <nav className="pointer-events-auto w-full bg-white/90 backdrop-blur-2xl border border-white/70 shadow-[0_14px_50px_rgba(15,23,42,0.12)] rounded-full px-2 py-2 flex items-center gap-1">
            ...
            {desktopLinks.map(({ path, label, icon: Icon }) => {
              ...
              return (
                <Link
                  key={path}
                  to={path}
                  className={`relative px-4 lg:px-5 py-2.5 rounded-full text-sm font-bold transition-colors ${active ? 'text-slate-950' : 'text-slate-500 hover:text-slate-800'}`}
                >
                  {active && (
                    <motion.span
          ...
          <Link
            to="/input"
            className="inline-flex items-center gap-2 rounded-full bg-slate-950 text-white px-5 py-2.5 text-sm font-bold shadow-lg shadow-slate-900/15 hover:bg-black transition-colors"
          >
            <CirclePlus size={17} /> Catat
          </Link>

          <button
            onClick={handleLogout}
            className="ml-1 inline-flex items-center gap-2 rounded-full px-4 py-2.5 text-sm font-bold text-rose-500 hover:bg-rose-50 transition-colors"
          >
            <LogOut size={17} />
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-50 bg-white/95 backdrop-blur-2xl border-t border-slate-200 shadow-[0_-12px_35px_rgba(15,23,42,0.08)] px-3 pt-2 pb-[calc(0.65rem+env(safe-area-inset-bottom))]">
        <div className="grid grid-cols-5 items-end">
          {mobileLinks.map(({ path, label, icon: Icon, isFab }) => {
            ...
            if (isFab) {
              return (
                <Link key={path} to={path} className="flex flex-col items-center justify-end gap-1 relative -top-4">
                  <motion.span whileTap={{ scale: 0.92 }} className="w-14 h-14 rounded-full bg-slate-950 text-white flex items-center justify-center ring-4 ring-white shadow-lg shadow-slate-900/20">
                    <Icon size={27} />
                  </motion.span>
            ...
            return (
              <Link key={path} to={path} className={`flex flex-col items-center justify-end gap-1 py-1.5 ${active ? 'text-slate-950' : 'text-slate-400'}`}>
                <span className={`w-9 h-9 rounded-2xl flex items-center justify-center transition-colors ${active ? 'bg-slate-100' : ''}`}>
